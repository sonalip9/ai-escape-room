...
vi.mock('@/utils/puzzles', () => ({
  fallbackPuzzles: [{ id: 'test', type: 'riddle', question: 'Test puzzle', normalized_answers: ['answer'] }],
...
describe('Rate limiting integration across all API endpoints', () => {
  ...
  describe('Rate limit headers', () => {
    it('includes proper rate limit headers in all responses', async () => {
      ...
      expect(response.headers.get('X-RateLimit-Limit')).toBe(RATE_LIMIT_CONFIG.maxRequests.toString());
